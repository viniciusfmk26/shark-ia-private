...
~~**1 ocorrência mantida** propositalmente em `app/(dashboard)/templates/page.tsx:116`: `{ key: 'emerald', label: 'Verde', classes: 'bg-cyan-500/10 ...' }`. É um color picker em que a `key` é persistida no banco em templates de usuários — renomear quebraria templates já criados. Classes mapeadas já são cyan; `label: 'Verde'` ficou inconsistente com a cor renderizada (renomear o label é decisão de produto, fora do escopo).~~ — **Resolvido em 28/04/2026** (commit `2b33ebd5`): scan das 8 tabelas com coluna `color` (`contact_tags`, `funnel_stages`, `guided_funnel_templates`, `knowledge_categories`, `quick_trigger_config`, `sales_funnel_config`, `tags`, `template_categories`) confirmou **0 linhas** com `color='emerald'` em produção. `UPDATE template_categories SET color='cyan' WHERE color='emerald'` rodado como invariante (no-op). Entrada `emerald` removida de `COLOR_OPTIONS` (`cyan` já existia com classes idênticas) — picker deixou de mostrar duas opções visualmente iguais.
