...
- **CUIDADO com IDs trocados (incidente 15/09):** na instância `Marlene SK` os campos estavam invertidos (`cloud_phone_number_id` recebeu o WABA ID). Sempre validar: `GET /{waba_id}/phone_numbers?fields=id,display_phone_number` responde os phone ids; WABA ID `1608150587414256` ≠ phone id. Números com prefixo `+1 555` são **test number** da Meta — nunca usar em produção; usar o número real da lista (`+55 22 99875-4276`).
